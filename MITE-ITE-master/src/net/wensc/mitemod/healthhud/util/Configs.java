package net.wensc.mitemod.healthhud.util;
...
public class Configs {
    ...
    public static class wenscConfig {
        public static ConfigItem <Integer> healthHalfWidth = new ConfigItem("healthHalfWidth", 20, "血条半距宽度", 0, 1000);
    }

    public static void loadConfigs(){

        wenscMap.put("healthHalfWidth", wenscConfig.healthHalfWidth);

        String filePth = "mite-health-hud.cfg";
        ...
        if (file_mite.exists()) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(jFrame, "mite-health-hud.cfg配置文件失败，请前往www.wensc.cn自行下载", "错误", 0);
                System.exit(0);
            }
        }
    }
    ...
    public static void generateConfigFile(File file) {
        try{
            ...
            fileWritter.write("// MITE血条显示配置文件，说明：【布尔类型：true为开启，false关闭】，在【名称=值】之间/之后不要存在空格或者其他无关字符\n\n");
            for (Map.Entry<String, ConfigItem> entry: wenscMap.entrySet()) {
                ConfigItem value = entry.getValue();
                fileWritter.write("// " + value.ConfigComment + "\n");
                fileWritter.write(value.ConfigKey + "=" + value.ConfigValue + "\n\n");
